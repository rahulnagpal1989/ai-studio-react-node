...
import {
  createGeneration as createGenerationModel,
...
const createGenerationSchema = Joi.object({
  ...
  style: Joi.string()
    .valid('Classic', 'Avant-garde', 'Street')
    .required()
    .messages({
...
export async function createGeneration(req: Request, res: Response) {
  try {
    ...
    const generationId = await createGenerationModel(
      userId,
      prompt,
      style,
      imageUrl,
    ...
    res
      .status(500)
      .json({ message: 'There is some issue please try again later' });
  }
}
