...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.response?.data?.details?.join(', ') || 'Login failed';
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900/60 border border-slate-700 rounded-lg rounded-t-none p-4 md:p-8 shadow-lg">
            ...
            <form onSubmit={submit} className="grid gap-3" role="form" aria-label="Login form">
              <div>
                ...
                />
                <div id="login-email-help" className="sr-only">Enter your email address</div>
              </div>
              <div>
                ...
                />
                <div id="login-password-help" className="sr-only">Enter your password</div>
              </div>
              <div>
                <button 
                ...
                <div id="login-submit-help" className="sr-only">Sign in to your account</div>
              </div>
            </form>
          </div>
          <div className="bg-slate-900/60 border border-slate-700 rounded-lg rounded-t-none p-4 md:p-8 shadow-lg">
            <h2 className="text-white text-xl font-bold tracking-tight">Why AI Studio?</h2>
            <p className="text-gray-400">Turn prompts and references into polished visuals with one click.</p>
          </div>
        </div>
      </div>
    </AuthGate>
